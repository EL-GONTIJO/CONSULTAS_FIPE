...
def load_checkpoint():
    if os.path.exists("checkpoint.txt"):
        with open("checkpoint.txt") as file:
            marca_id, modelo_id, ano_codigo = file.read().split(",")
            return int(marca_id), int(modelo_id), ano_codigo.strip()
...
for marca in marcas:
    ...
    for modelo in modelos:
        ...
        for ano in anos:
            ...
            if ano_checkpoint is not None and ano_codigo <= ano_checkpoint:
                continue
